...
\begin{proposition}
    ...
    \begin{equation}
        \nabla_{p_i} A^r(p) = 2r \meanv{S}(p_i) Vol(S \cap V(p_i, P)) + O(\epsilon)
    \end{equation}
...
$ \vec{n_S}(q) = \vec{n_S}(p_i) $, $ \meanv{S}(q) = \meanv{S}(p_i) $. Error terms
are negligible since they depend on the derivatives of the curvature. We deduce
that:

$$ \nabla_{p_i} A^r(p) = 2r \meanv{S}(p_i) \int_{S \cap V(p_i, P)} 1 dq +
O(\epsilon) $$
